...
var CONFIG = {
  ...
  // ── INGRESOS FIJOS ────────────────────────────────────
  income: {
    nomina:              83096,   // neto mensual con offcycle (May+)
    nominaPreOffcycle:   79988,   // neto mensual sin offcycle (Abr)
    vales:               3292,    // desde Mayo
    offcycleActive:      false    // se activa en Junio
  },

  // ── GASTOS FIJOS ─────────────────────────────────────
  fixedExpenses: [
    { id:"renta",      label:"Renta + servicios",         amount:17250, category:"vivienda" },
    { id:"att",        label:"AT&T personal",             amount:532,   category:"telecom"  },
    { id:"bizSubs",    label:"Subs trabajo/Smability",    amount:1951,  category:"negocio"  },
    { id:"apple",      label:"Apple (iCloud ×2)",         amount:218,   category:"personal" },
    { id:"googleOne",  label:"Google One",                amount:395,   category:"personal" },
    { id:"amazon",     label:"Amazon Prime",              amount:75,    category:"personal", endsMonth:"2026-07" },
  ],
...
var CALC = {
  ...
  monthlyInterest: function(key) {
    ...
    return Math.round(d.balance * (d.rate / 100 / 12));
  },

  // Total gastos fijos del mes
  totalFixed: function(ym) {
    ym = ym || currentYM();
    return CONFIG.fixedExpenses.reduce(function(s,e){
      if (e.endsMonth && ym > e.endsMonth) return s;
      return s + (e.amount||0);
    }, 0);
  },
  ...
  // Ingreso extraordinario del mes (EI pendiente de alta probabilidad)
  eiForMonth: function(ym) {
    if (typeof EI === "undefined") return 0;
    return EI.totalForMonth(ym);
  },

  // Excedente real = ingresos - gastos fijos - variables - mínimos deuda
  surplusForMonth: function(ym) {
    ym = ym || currentYM();
    var income  = this.monthlyIncome(ym);
    var fixed   = this.totalFixed(ym);
    var variable= this.totalVariable();
    var mins    = this.totalMinPayments();
    return income - fixed - variable - mins;
  },
